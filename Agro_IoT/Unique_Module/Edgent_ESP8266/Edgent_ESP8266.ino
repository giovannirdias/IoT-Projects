...
const int IWF_SensorPin = 0;  // Pino interrupt que equivale ao pino 4

// Criar a instancia de classes (objetos)
DHT dht(DHTPIN, DHT22);   // Pode ser DHT11 tambem
SoftwareSerial ESP_wifi(2, 3);   // Pinos  RX | TX
...
void loop() {
    ...
    else if ((humidade_solo >= 0 && humidade_solo < 50) || (humidade < 40 || humidade > 80) ){
...
void water_flow_measure(){
    ...
    if ((millis() - before_time)>1000){
        ...
        Wcount = 0;
        before_time = millis();
        attachInterrupt(IWF_SensorPin, pulseCounter, FALLING);
    }
}
